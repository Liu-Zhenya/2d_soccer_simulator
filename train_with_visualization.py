...
def render_env(
    ...
    score_A,
    score_B,
    reward_window_A=None,
    reward_window_B=None,
):
    ...
    # Display game info
    macro_str_A = (
        MACRO_ACTIONS[macro_A] if macro_A >= 0 and macro_A < len(MACRO_ACTIONS) else "None"
    )
    macro_str_B = (
        MACRO_ACTIONS[macro_B] if macro_B >= 0 and macro_B < len(MACRO_ACTIONS) else "None"
    )
    ...
    if (reward_window_A is not None and len(reward_window_A) > 1) or (
        reward_window_B is not None and len(reward_window_B) > 1
    ):
        # Draw reward trend
        ...
        pygame.draw.rect(
            screen, (0, 0, 0, 128), (chart_x, chart_y, chart_width, chart_height)
        )
        ...
        if combined_rewards:
            ...
            if min_val <= 0 and max_val >= 0:
                zero_y = (
                    chart_y + chart_height - int((0 - min_val) / range_val * chart_height)
                )
                pygame.draw.line(
                    screen, GRAY, (chart_x, zero_y), (chart_x + chart_width, zero_y), 1
                )
            ...
            if reward_window_A is not None and len(reward_window_A) > 1:
                for i in range(1, len(reward_window_A)):
                    ...
                    y1 = (
                        chart_y
                        + chart_height
                        - int((reward_window_A[i - 1] - min_val) / range_val * chart_height)
                    )
                    x2 = chart_x + int(i * chart_width / (len(reward_window_A) - 1))
                    y2 = (
                        chart_y
                        + chart_height
                        - int((reward_window_A[i] - min_val) / range_val * chart_height)
                    )
            ...
            if reward_window_B is not None and len(reward_window_B) > 1:
                for i in range(1, len(reward_window_B)):
                    ...
                    y1 = (
                        chart_y
                        + chart_height
                        - int((reward_window_B[i - 1] - min_val) / range_val * chart_height)
                    )
                    x2 = chart_x + int(i * chart_width / (len(reward_window_B) - 1))
                    y2 = (
                        chart_y
                        + chart_height
                        - int((reward_window_B[i] - min_val) / range_val * chart_height)
                    )
                    pygame.draw.line(screen, RED, (x1, y1), (x2, y2), 2)

            # Legend
            pygame.draw.line(
                screen, BLUE, (chart_x + 10, chart_y - 15), (chart_x + 30, chart_y - 15), 2
            )
            screen.blit(FONT.render("A", True, WHITE), (chart_x + 35, chart_y - 20))
            pygame.draw.line(
                screen, RED, (chart_x + 50, chart_y - 15), (chart_x + 70, chart_y - 15), 2
            )
...
def train_with_visualization(env, agent_A, agent_B, num_episodes, fps=30):
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        while paused and running:
                            ...
                            screen.blit(
                                FONT.render(
                                    "PAUSED - Press SPACE to continue", True, WHITE
                                ),
    ...
    if window_size > 0:
        smoothed_A = [
            sum(x[0] for x in reward_tracker[i : i + window_size]) / window_size
            for i in range(len(reward_tracker) - window_size + 1)
        ]
        smoothed_B = [
            sum(x[1] for x in reward_tracker[i : i + window_size]) / window_size
            for i in range(len(reward_tracker) - window_size + 1)
        ]
